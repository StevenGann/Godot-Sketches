...
var collision_objects: Dictionary[ScatterShotMeshes, ScatterShotCollisionObject] = {}
...
func items_begin(scatter_zone: ScatterShotZone) -> void:
	zone = scatter_zone
...
func _add_mesh(collection: ScatterShotMeshes, item_index: int, transform: Transform3D) -> void:
	...
	active_body_count += 1
	PhysicsServer3D.body_attach_object_instance_id(body_rid, _collision_object(collection).get_instance_id())
...
func _collision_object(collection: ScatterShotMeshes) -> ScatterShotCollisionObject:
	var untyped := collision_objects.get(collection)
	if not untyped:
		var d: ScatterShotCollisionObject = ScatterShotCollisionObject.new(zone, collection)
		untyped = d
		collision_objects[collection] = d
	return untyped
